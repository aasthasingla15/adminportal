...
export default function AdminEditEventPage({ initialEvent }) {
  const [title, setTitle] = useState(initialEvent.title || '');
  const [description, setDescription] = useState(initialEvent.description || '');
  const [date, setDate] = useState(initialEvent.date || '');
  const [time, setTime] = useState(initialEvent.time || '');
  const [venue, setVenue] = useState(initialEvent.venue || '');
  const [category, setCategory] = useState(initialEvent.category || 'Workshop');
  const [bannerImage, setBannerImage] = useState(initialEvent.bannerImage || '');
